...
class Google_OAuth2 extends Google_Auth {
  ...
  const OAUTH2_TOKENINFO_URI = 'https://www.googleapis.com/oauth2/v3/tokeninfo';
  ...
  public function tokeninfo($idToken) {
    $http = new Google_HttpRequest(self::OAUTH2_TOKENINFO_URI, 'POST', array(), array('id_token' => $idToken));
    ...
    if (200 == $code) {
      ...
      if ($token == null) {
        throw new Google_AuthException("Could not json decode the access tokeninfo");
      }
	  return $token;
    } else {
      throw new Google_AuthException("Error refreshing the OAuth2 tokeninfo, message: '$body'", $code);
    }
  }
