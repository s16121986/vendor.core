...
class Sender {
	...
	protected function SmtpSend($header, $body) {
		...
		if (count($bad_rcpt) > 0) { //Create error message for any bad addresses
			...
			throw new Exception($this->Lang('recipients_failed') . $badaddresses);
		}
		...
		if ($this->IsError()) {
			...
			try {
				if (!defined('PKCS7_TEXT')) {
					throw new Exception($this->Lang('signing') . ' OpenSSL extension missing.');
				}
				$file = tempnam(sys_get_temp_dir(), 'mail');
				file_put_contents($file, $body); //TODO check this worked
				$signed = tempnam(sys_get_temp_dir(), 'signed');
				if (@openssl_pkcs7_sign($file, $signed, 'file://' . realpath($this->sign_cert_file), ['file://' . realpath($this->sign_key_file), $this->sign_key_pass], null)) {
					@unlink($file);
	...
	public function set($name, $value = '') {
		try {
			if (isset($this->$name)) {
				...
				throw new Exception($this->Lang('variable_set') . $name, self::STOP_CRITICAL);
			}
		} catch (Exception $e) {
			$this->SetError($e->getMessage());
